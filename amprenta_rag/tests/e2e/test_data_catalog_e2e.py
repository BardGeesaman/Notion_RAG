...
def test_basic_page_structure(catalog_page: Page):
    ...
    # Test Browse Entities tab (active by default)
    expect(main.get_by_role("heading", name="Browse Data Entities")).to_be_visible()
...
def test_api_integration_works(catalog_page: Page):
    ...
    # Check for Browse Entities tab elements (indicates page rendered)
    expect(main.get_by_text("Entity Type")).to_be_visible()  # Filter label always present
