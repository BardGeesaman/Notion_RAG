...
def test_link_features_to_dataset_appends(monkeypatch):
    ...
    # Avoid real creation by forcing _find_or_create_feature_impl
...
def test_link_features_updates_metadata(monkeypatch):
    ...
    assert "f1" in record.semantic_metadata["feature_mentions"] or "F1" in record.semantic_metadata["feature_mentions"]
...
def test_batch_link_features_to_dataset(monkeypatch):
    ...
    # Use real FeatureModel path but simplified: create dummy objects
    class FakeFeatureModel:
        def __init__(self, name, feature_type):
            self.name = name
            self.feature_type = feature_type
