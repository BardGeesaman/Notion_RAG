"""
Tests for flow cytometry API endpoints.

This test suite covers all 9 flow cytometry API endpoints including:
- FCS file upload and ingestion
- Dataset listing and retrieval
- Event data access with pagination
- Gate CRUD operations
- Population statistics
"""

import tempfile
from io import BytesIO
from pathlib import Path
from unittest.mock import MagicMock, patch
from uuid import uuid4

import pytest
from fastapi.testclient import TestClient

from amprenta_rag.api.main import app
from amprenta_rag.api.schemas import GateCreate


class TestFlowCytometryAPI:
    """Test flow cytometry API endpoints."""
    
    @pytest.fixture
    def client(self):
        """Create test client with auth disabled."""
        with patch.dict("os.environ", {"DISABLE_AUTH": "true"}):
            with TestClient(app) as test_client:
                yield test_client
    
    @pytest.fixture
    def mock_fcs_file(self):
        """Create a mock FCS file for upload testing."""
        # Create a simple mock FCS file content
        content = b"FCS3.0    256     512    1024    2048    0    0    0    0    0" + b"\x00" * 200
        content += b"mock event data" * 100  # Simulate event data
        
        return BytesIO(content)
    
    @pytest.fixture
    def mock_flow_dataset(self):
        """Mock FlowCytometryDataset for testing."""
        dataset = MagicMock()
        dataset.id = uuid4()
        dataset.dataset_id = uuid4()
        dataset.events_parquet_path = "test_events.parquet"
        dataset.file_size_bytes = 1024
        dataset.n_events = 1000
        dataset.n_parameters = 10
        dataset.processing_status = "completed"
        dataset.processing_log = None
        dataset.sample_id = "TEST001"
        return dataset
    
    @pytest.fixture
    def mock_gate(self):
        """Mock FlowCytometryGate for testing."""
        gate = MagicMock()
        gate.id = uuid4()
        gate.flow_dataset_id = uuid4()
        gate.gate_name = "Test Gate"
        gate.gate_type = "rectangle"
        gate.gate_definition = {"x_min": 0, "x_max": 1000, "y_min": 0, "y_max": 1000}
        gate.x_parameter_id = uuid4()
        gate.y_parameter_id = uuid4()
        gate.is_active = True
        return gate
    
    def test_upload_fcs_file_success(self, client, mock_fcs_file):
        """Test successful FCS file upload."""
        with patch('amprenta_rag.api.routers.flow_cytometry.ingest_fcs') as mock_ingest:
            # Setup mock
            mock_dataset = MagicMock()
            mock_dataset.id = uuid4()
            mock_dataset.dataset_id = uuid4()
            mock_dataset.processing_status = "pending"
            mock_ingest.return_value = mock_dataset
            
            # Test upload
            response = client.post(
                "/api/v1/flow-cytometry/upload",
                files={"file": ("test.fcs", mock_fcs_file, "application/octet-stream")}
            )
            
            assert response.status_code == 201
            data = response.json()
            assert "flow_dataset_id" in data
            assert "dataset_id" in data
            assert data["filename"] == "test.fcs"
            assert data["processing_status"] == "pending"
            assert "message" in data
            
            # Verify ingest was called
            mock_ingest.assert_called_once()
    
    def test_upload_fcs_file_invalid_extension(self, client):
        """Test FCS upload with invalid file extension."""
        mock_file = BytesIO(b"test data")
        
        response = client.post(
            "/api/v1/flow-cytometry/upload",
            files={"file": ("test.txt", mock_file, "text/plain")}
        )
        
        assert response.status_code == 400
        assert "must have .fcs extension" in response.json()["detail"]
    
    def test_list_flow_datasets(self, client, mock_flow_dataset):
        """Test listing flow cytometry datasets."""
        with patch('amprenta_rag.api.routers.flow_cytometry.FlowCytometryDataset') as mock_model:
            # Mock the query chain
            mock_query = MagicMock()
            mock_query.order_by.return_value.offset.return_value.limit.return_value.all.return_value = [mock_flow_dataset]
            
            with patch('amprenta_rag.api.routers.flow_cytometry.get_database_session') as mock_db_session:
                mock_db = MagicMock()
                mock_db.query.return_value = mock_query
                mock_db_session.return_value.__enter__.return_value = mock_db
                
                # Test request
                response = client.get("/api/v1/flow-cytometry/datasets")
                
                assert response.status_code == 200
                data = response.json()
                assert isinstance(data, list)
                assert len(data) == 1
    
    @patch('amprenta_rag.api.routers.flow_cytometry.get_database_session')
    def test_list_flow_datasets_with_filters(self, mock_db_session, client, mock_flow_dataset):
        """Test listing datasets with status filter."""
        # Setup mock
        mock_db = MagicMock()
        mock_db_session.return_value.__enter__.return_value = mock_db
        
        # Setup query chain with filter
        mock_db.query.return_value.order_by.return_value.filter.return_value.offset.return_value.limit.return_value.all.return_value = [mock_flow_dataset]
        
        # Test with status filter
        response = client.get("/api/v1/flow-cytometry/datasets?processing_status=completed")
        
        assert response.status_code == 200
        data = response.json()
        assert len(data) == 1
    
    @patch('amprenta_rag.api.routers.flow_cytometry.get_database_session')
    def test_get_flow_dataset_success(self, mock_db_session, client, mock_flow_dataset):
        """Test getting specific flow dataset."""
        # Setup mock
        mock_db = MagicMock()
        mock_db_session.return_value.__enter__.return_value = mock_db
        
        # Setup the query chain
        query_mock = MagicMock()
        filter_mock = MagicMock()
        query_mock.filter.return_value = filter_mock
        filter_mock.first.return_value = mock_flow_dataset
        mock_db.query.return_value = query_mock
        
        dataset_id = mock_flow_dataset.id
        
        # Test request
        response = client.get(f"/api/v1/flow-cytometry/datasets/{dataset_id}")
        
        assert response.status_code == 200
        data = response.json()
        assert data["id"] == str(dataset_id)
        assert data["processing_status"] == "completed"
    
    @patch('amprenta_rag.api.routers.flow_cytometry.get_database_session')
    def test_get_flow_dataset_not_found(self, mock_db_session, client):
        """Test getting non-existent dataset."""
        # Setup mock
        mock_db = MagicMock()
        mock_db_session.return_value.__enter__.return_value = mock_db
        mock_db.query.return_value.filter.return_value.first.return_value = None
        
        dataset_id = uuid4()
        
        # Test request
        response = client.get(f"/api/v1/flow-cytometry/datasets/{dataset_id}")
        
        assert response.status_code == 404
        assert "not found" in response.json()["detail"]
    
    @patch('amprenta_rag.api.routers.flow_cytometry.get_database_session')
    def test_get_dataset_parameters(self, mock_db_session, client, mock_flow_dataset):
        """Test getting dataset parameters."""
        # Setup mock
        mock_db = MagicMock()
        mock_db_session.return_value.__enter__.return_value = mock_db
        
        # Mock dataset exists
        mock_db.query.return_value.filter.return_value.first.return_value = mock_flow_dataset
        
        # Mock parameters
        mock_param = MagicMock()
        mock_param.id = uuid4()
        mock_param.parameter_index = 0
        mock_param.parameter_name = "FSC-A"
        mock_param.fluorophore = None
        
        mock_db.query.return_value.filter.return_value.order_by.return_value.all.return_value = [mock_param]
        
        dataset_id = mock_flow_dataset.id
        
        # Test request
        response = client.get(f"/api/v1/flow-cytometry/datasets/{dataset_id}/parameters")
        
        assert response.status_code == 200
        data = response.json()
        assert isinstance(data, list)
        assert len(data) == 1
        assert data[0]["parameter_name"] == "FSC-A"
    
    @patch('amprenta_rag.api.routers.flow_cytometry.load_events_parquet')
    @patch('amprenta_rag.api.routers.flow_cytometry.get_database_session')
    def test_get_dataset_events(self, mock_db_session, mock_load_events, client, mock_flow_dataset):
        """Test getting dataset events."""
        # Setup mocks
        mock_db = MagicMock()
        mock_db_session.return_value.__enter__.return_value = mock_db
        mock_db.query.return_value.filter.return_value.first.return_value = mock_flow_dataset
        
        # Mock events data
        import numpy as np
        mock_events = np.array([[100, 200], [150, 250], [200, 300]], dtype=np.float32)
        mock_params = ["FSC-A", "SSC-A"]
        mock_load_events.return_value = (mock_events, mock_params)
        
        dataset_id = mock_flow_dataset.id
        
        # Test request
        response = client.get(f"/api/v1/flow-cytometry/datasets/{dataset_id}/events?limit=10")
        
        assert response.status_code == 200
        data = response.json()
        assert "events" in data
        assert "parameter_names" in data
        assert data["parameter_names"] == mock_params
        assert data["total_events"] == 3
        assert len(data["events"]) == 3
    
    @patch('amprenta_rag.api.routers.flow_cytometry.get_database_session')
    def test_get_dataset_events_not_processed(self, mock_db_session, client):
        """Test getting events from unprocessed dataset."""
        # Setup mock for unprocessed dataset
        mock_db = MagicMock()
        mock_db_session.return_value.__enter__.return_value = mock_db
        
        mock_dataset = MagicMock()
        mock_dataset.processing_status = "pending"
        mock_db.query.return_value.filter.return_value.first.return_value = mock_dataset
        
        dataset_id = uuid4()
        
        # Test request
        response = client.get(f"/api/v1/flow-cytometry/datasets/{dataset_id}/events")
        
        assert response.status_code == 409
        assert "processing not completed" in response.json()["detail"]
    
    @patch('amprenta_rag.api.routers.flow_cytometry.apply_gate_to_dataset')
    @patch('amprenta_rag.api.routers.flow_cytometry.get_database_session')
    def test_create_gate_success(self, mock_db_session, mock_apply_gate, client, mock_flow_dataset, mock_gate):
        """Test successful gate creation."""
        # Setup mocks
        mock_db = MagicMock()
        mock_db_session.return_value.__enter__.return_value = mock_db
        
        # Mock dataset and parameters exist
        mock_db.query.return_value.filter.return_value.first.side_effect = [
            mock_flow_dataset,  # Dataset query
            MagicMock(),        # X parameter query
            MagicMock()         # Y parameter query
        ]
        
        mock_apply_gate.return_value = mock_gate
        
        dataset_id = mock_flow_dataset.id
        
        # Test gate creation
        gate_data = {
            "gate_name": "Test Rectangle Gate",
            "gate_type": "rectangle",
            "gate_definition": {"x_min": 0, "x_max": 1000, "y_min": 0, "y_max": 1000},
            "x_parameter_id": str(uuid4()),
            "y_parameter_id": str(uuid4())
        }
        
        response = client.post(
            f"/api/v1/flow-cytometry/datasets/{dataset_id}/gates",
            json=gate_data
        )
        
        assert response.status_code == 201
        data = response.json()
        assert data["gate_name"] == "Test Gate"
        assert data["gate_type"] == "rectangle"
        
        # Verify apply_gate_to_dataset was called
        mock_apply_gate.assert_called_once()
    
    @patch('amprenta_rag.api.routers.flow_cytometry.get_database_session')
    def test_create_gate_invalid_definition(self, mock_db_session, client, mock_flow_dataset):
        """Test gate creation with invalid definition."""
        # Setup mocks
        mock_db = MagicMock()
        mock_db_session.return_value.__enter__.return_value = mock_db
        mock_db.query.return_value.filter.return_value.first.return_value = mock_flow_dataset
        
        dataset_id = mock_flow_dataset.id
        
        # Test with invalid polygon gate (< 3 vertices)
        gate_data = {
            "gate_name": "Invalid Polygon",
            "gate_type": "polygon",
            "gate_definition": {"vertices": [[0, 0], [1, 1]]},  # Only 2 vertices
            "x_parameter_id": str(uuid4()),
            "y_parameter_id": str(uuid4())
        }
        
        response = client.post(
            f"/api/v1/flow-cytometry/datasets/{dataset_id}/gates",
            json=gate_data
        )
        
        assert response.status_code == 422  # Validation error
        assert "at least 3 vertices" in str(response.json())
    
    @patch('amprenta_rag.api.routers.flow_cytometry.get_database_session')
    def test_list_gates(self, mock_db_session, client, mock_flow_dataset, mock_gate):
        """Test listing gates for a dataset."""
        # Setup mocks
        mock_db = MagicMock()
        mock_db_session.return_value.__enter__.return_value = mock_db
        
        # Mock dataset exists
        mock_db.query.return_value.filter.return_value.first.return_value = mock_flow_dataset
        
        # Mock gates query
        mock_db.query.return_value.filter.return_value.filter.return_value.order_by.return_value.all.return_value = [mock_gate]
        
        dataset_id = mock_flow_dataset.id
        
        # Test request
        response = client.get(f"/api/v1/flow-cytometry/datasets/{dataset_id}/gates")
        
        assert response.status_code == 200
        data = response.json()
        assert isinstance(data, list)
        assert len(data) == 1
        assert data[0]["gate_name"] == "Test Gate"
    
    @patch('amprenta_rag.api.routers.flow_cytometry.get_database_session')
    def test_update_gate(self, mock_db_session, client, mock_gate):
        """Test updating an existing gate."""
        # Setup mocks
        mock_db = MagicMock()
        mock_db_session.return_value.__enter__.return_value = mock_db
        mock_db.query.return_value.filter.return_value.first.return_value = mock_gate
        
        gate_id = mock_gate.id
        
        # Test update
        update_data = {
            "gate_name": "Updated Gate Name",
            "is_active": False
        }
        
        response = client.put(
            f"/api/v1/flow-cytometry/gates/{gate_id}",
            json=update_data
        )
        
        assert response.status_code == 200
        data = response.json()
        assert data["gate_name"] == "Test Gate"  # Mock returns original name
        
        # Verify database operations
        mock_db.add.assert_called()
        mock_db.commit.assert_called()
    
    @patch('amprenta_rag.api.routers.flow_cytometry.get_database_session')
    def test_delete_gate(self, mock_db_session, client, mock_gate):
        """Test deleting (deactivating) a gate."""
        # Setup mocks
        mock_db = MagicMock()
        mock_db_session.return_value.__enter__.return_value = mock_db
        mock_db.query.return_value.filter.return_value.first.return_value = mock_gate
        
        gate_id = mock_gate.id
        
        # Test delete
        response = client.delete(f"/api/v1/flow-cytometry/gates/{gate_id}")
        
        assert response.status_code == 204
        
        # Verify gate was marked inactive
        assert mock_gate.is_active == False
        mock_db.add.assert_called()
        mock_db.commit.assert_called()
    
    @patch('amprenta_rag.api.routers.flow_cytometry.get_database_session')
    def test_delete_gate_not_found(self, mock_db_session, client):
        """Test deleting non-existent gate."""
        # Setup mocks
        mock_db = MagicMock()
        mock_db_session.return_value.__enter__.return_value = mock_db
        mock_db.query.return_value.filter.return_value.first.return_value = None
        
        gate_id = uuid4()
        
        # Test delete
        response = client.delete(f"/api/v1/flow-cytometry/gates/{gate_id}")
        
        assert response.status_code == 404
        assert "not found" in response.json()["detail"]
    
    @patch('amprenta_rag.api.routers.flow_cytometry.get_database_session')
    def test_get_population_statistics(self, mock_db_session, client, mock_flow_dataset):
        """Test getting population statistics."""
        # Setup mocks
        mock_db = MagicMock()
        mock_db_session.return_value.__enter__.return_value = mock_db
        
        # Mock dataset exists
        mock_db.query.return_value.filter.return_value.first.return_value = mock_flow_dataset
        
        # Mock population data
        mock_population = MagicMock()
        mock_population.id = uuid4()
        mock_population.event_count = 500
        mock_population.percentage_of_total = 50.0
        mock_population.parameter_statistics = {"FSC-A": {"mean": 1000, "median": 950}}
        
        mock_db.query.return_value.filter.return_value.order_by.return_value.all.return_value = [mock_population]
        
        dataset_id = mock_flow_dataset.id
        
        # Test request
        response = client.get(f"/api/v1/flow-cytometry/datasets/{dataset_id}/populations")
        
        assert response.status_code == 200
        data = response.json()
        assert isinstance(data, list)
        assert len(data) == 1
        assert data[0]["event_count"] == 500
        assert data[0]["percentage_of_total"] == 50.0


if __name__ == "__main__":
    pytest.main([__file__, "-v"])
