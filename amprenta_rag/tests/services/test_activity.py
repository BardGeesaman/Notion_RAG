...
class TestActivityService:
    ...
    def test_log_activity_with_program_routes_notification(self):
        ...
        # Create a user to be program lead
        lead_id = uuid4()
        with db_session() as db:
            user = User(id=lead_id, username="program_lead", email="lead@test.com", password_hash="dummy_hash")
            db.add(user)
            db.commit()
    ...
    def test_route_notifications_excludes_actor(self):
        ...
        # Create a user who is both actor and program lead
        user_id = uuid4()
        with db_session() as db:
            user = User(id=user_id, username="test_user", email="user@test.com", password_hash="dummy_hash")
            db.add(user)
            db.commit()
    ...
    def test_mark_notification_read(self):
        ...
        # Setup user, program, and notification
        user_id = uuid4()
        with db_session() as db:
            user = User(id=user_id, username="test_user", email="user@test.com", password_hash="dummy_hash")
            db.add(user)
    ...
    def test_log_activity_with_mocks(self, mock_db_session):
        ...
        with patch('amprenta_rag.services.activity.route_notifications') as mock_route:
            ...
            # (result will be None due to mocking, but that's expected)
