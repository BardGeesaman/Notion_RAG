...
def notion_headers() -> Dict[str, str]:
    ...
    logger.debug("[TRANSCRIPTOMICS][INGESTION] notion_headers() deprecated - Notion support removed")
    return {}

def fetch_dataset_page(page_id: str) -> Dict[str, Any]:
    """DEPRECATED: Notion support removed. Returns empty dict."""
    logger.debug("[TRANSCRIPTOMICS][INGESTION] fetch_dataset_page() deprecated - Notion support removed")
    ...
    if cfg.pipeline.enable_notion_sync or cfg.pipeline.enable_dual_write:
        try:
            if notion_page_id:
                ...
                logger.info(
                    "[INGEST][TRANSCRIPTOMICS] Using existing Notion dataset page %s",
                    page_id,
                )
                # Update summary
                try:
                    ...
                    logger.warning(
                        "[INGEST][TRANSCRIPTOMICS] Could not update Notion summary for page %s: %r",
                        page_id,
                        e,
                    )
            elif create_page:
                ...
                if postgres_dataset:
                    ...
                    db.commit()
                    logger.info(
                        "[INGEST][TRANSCRIPTOMICS] Linked Postgres dataset %s to Notion page %s",
                        postgres_dataset.id,
                        page_id,
                    )
            ...
            logger.warning(
                "[INGEST][TRANSCRIPTOMICS] Notion sync skipped (error): %r",
                e,
            )
    elif not cfg.pipeline.use_postgres_as_sot:
