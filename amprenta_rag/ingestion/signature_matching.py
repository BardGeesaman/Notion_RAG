...
def fetch_all_signatures_from_notion() -> List[Dict[str, Any]]:
    ...
    if not cfg.notion.signature_db_id:
        logger.warning(
            "[INGEST][SIGNATURE-MATCH] Signature database ID not configured"
        )
        return []
...
def load_signature_from_notion_page(signature_page: Dict[str, Any]) -> Optional[Signature]:
    """
    ...
    if not hasattr(cfg.notion, 'signature_component_db_id') or not cfg.notion.signature_component_db_id:
        logger.debug(
...
def find_matching_signatures_for_dataset(
    ...
    for sig_page in signature_pages:
        try:
            ...
            overlap_fraction = matched_count / total_components if total_components > 0 else 0.0
...
def update_dataset_with_signature_matches(
    ...
    try:
        ...
        try:
            ...
            existing_summary = "".join(rt.get("plain_text", "") for rt in existing_summary_rich)
