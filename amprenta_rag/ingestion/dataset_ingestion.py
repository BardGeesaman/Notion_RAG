...
def _extract_mwtab_from_page_content(page_content: str) -> Optional[Dict[str, Any]]:
    ...
    if first_brace >= 0:
        logger.debug("[INGEST][MWTAB] Found code block with '{', attempting JSON parse...")
            ...
            try:
                ...
                if isinstance(data, dict):
                    ...
                    if any(indicator in str(data) for indicator in mwtab_indicators):
                        logger.info("[INGEST][MWTAB] Successfully parsed mwTab JSON from page content")
                        return data
        ...
        if json_end > first_brace:
            try:
                ...
                if isinstance(data, dict):
                    if any(indicator in str(data) for indicator in mwtab_indicators):
                        logger.info("[INGEST][MWTAB] Successfully parsed mwTab JSON from page content (brace-counting)")
                        return data
            except (json.JSONDecodeError, ValueError) as e:
                logger.debug("[INGEST][MWTAB] Failed JSON parse on block: %r", str(e)[:100])
...
def _extract_metadata_from_mwtab(mwtab_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "SUBJECT" in mwtab_data:
        ...
        if isinstance(subject, dict):
            ...
            subject_type = subject.get("SUBJECT_TYPE") or subject.get("subject_type") or ""
            if subject_type:
    ...
    if "STUDY" in mwtab_data:
        ...
        if isinstance(study, dict):
            ...
            if study_title:
                ...
                disease_patterns = [
                    r'\bfxs\b', r'\bfragile x\b', r'\bfragile-x\b',
                    r'\bals\b', r'\bamyotrophic\b',
                    r'\balzheimer\b', r'\bad\b',
                    r'\bparkinson\b', r'\bpd\b',
                    r'\bhuntington\b',
                ]
    ...
    if "TREATMENT" in mwtab_data:
        ...
        if isinstance(treatment, dict):
            treatment_summary = treatment.get("TREATMENT_SUMMARY") or treatment.get("summary") or ""
    ...
    if "SAMPLEPREP" in mwtab_data:
        ...
        if isinstance(sampleprep, dict):
            sampleprep_summary = sampleprep.get("SAMPLEPREP_SUMMARY") or sampleprep.get("summary") or ""
            if sampleprep_summary:
                methods_parts.append(f"Sample Preparation: {sampleprep_summary.strip()}")
    ...
    if "MS_METABOLITE_DATA" in mwtab_data:
        ...
        if isinstance(ms_data, dict):
            ...
            if isinstance(data_array, list) and len(data_array) > 0:
                metadata["results"] = f"Metabolite profiling data with {len(data_array)} metabolites detected."
    ...
    if "PROJECT" in mwtab_data and isinstance(mwtab_data["PROJECT"], dict):
        ...
        for field_pattern in ["project_website", "project_doi", "project_url", "doi", "url"]:
...
def _update_experimental_data_asset_metadata(
    ...
    if metadata.get("conclusions"):
        props["Conclusions"] = {
            "rich_text": [{"type": "text", "text": {"content": metadata["conclusions"]}}]
        }
    
    # Data Origin (select)
    if metadata.get("data_origin"):
        props["Data Origin"] = {
            "select": {"name": metadata["data_origin"]}
        }
...
def ingest_dataset(page_id: str, force: bool = False) -> None:
    ...
    try:
        ...
        if not mwtab_data:
            logger.info("[INGEST][MWTAB] Using fallback MW API fetch (page content extraction failed)")
            ...
            if not study_id:
                name_prop = props.get("Name", {}) or props.get("Experiment Name", {}) or {}
            ...
            if study_id:
                logger.info("[INGEST][MWTAB] Attempting MW API fallback fetch for STUDY_ID: %s", study_id)
                try:
                    ...
                    if mwtab_text:
                        logger.debug("[INGEST][MWTAB] Fetched mwTab text from MW API (%d chars)", len(mwtab_text))
                        ...
                        if first_brace >= 0:
                            ...
                            for end_offset in range(len(remaining), 100, -100):
                                try:
                                    json_str = remaining[:end_offset].strip().rstrip(',').rstrip()
                                    mwtab_data = json.loads(json_str)
                                    if isinstance(mwtab_data, dict):
                                        logger.info("[INGEST][MWTAB] MW API fallback fetch successful. Parsed mwTab JSON.")
                                        break
                            ...
                            if not mwtab_data:
                                ...
                                if json_end > first_brace:
                                    try:
                                        ...
                                        mwtab_data = json.loads(json_str)
                                        if isinstance(mwtab_data, dict):
                                            logger.info("[INGEST][MWTAB] MW API fallback fetch successful. Parsed mwTab JSON (brace-counting).")
                                    except (json.JSONDecodeError, ValueError) as parse_err:
                                        logger.warning("[INGEST][MWTAB] Failed to parse mwTab JSON from MW API response: %r", str(parse_err)[:200])
                        else:
                            logger.warning("[INGEST][MWTAB] MW API response does not contain JSON (no '{' found)")
                    else:
                        logger.warning("[INGEST][MWTAB] MW API fallback fetch returned empty response")
                    ...
                    if hasattr(fetch_error, 'response') and fetch_error.response is not None:
                        logger.warning(
                            "[INGEST][MWTAB] HTTP Status: %s, Response: %s",
                            fetch_error.response.status_code,
                            fetch_error.response.text[:500] if hasattr(fetch_error.response, 'text') else 'N/A',
                        )
            else:
                logger.info("[INGEST][MWTAB] Could not extract STUDY_ID from page properties or content for MW API fallback")
    except Exception as e:
        logger.warning(
    ...
    if mwtab_data:
        try:
            ...
            if has_metadata:
                ...
                logger.info("[INGEST][DATASET] No extractable metadata found in mwTab for %s", page_id)
        except Exception as e:
            logger.warning(
    ...
    if mwtab_data:
        try:
            ...
            if feature_names:
                ...
                logger.debug("[INGEST][DATASET] No metabolite features extracted from mwTab for %s", page_id)
        except Exception as e:
            logger.warning(
    ...
    if cfg.pipeline.enable_signature_scoring and mwtab_data:
        try:
            ...
            metabolite_sections = ["MS_METABOLITE_DATA", "GC_METABOLITE_DATA", "LC_METABOLITE_DATA", "METABOLITE_DATA"]
                    ...
                    if isinstance(data_array, list):
                        for row in data_array:
                            if isinstance(row, dict):
                                ...
                                for key in row.keys():
                                    if key.lower() in ["metabolite", "metabolite_name", "compound", "name"]:
                                        ...
                                        if raw_name and isinstance(raw_name, str):
                                            ...
                                            if raw_name:
                                                ...
                                                dataset_species_set.add(canonical if canonical else raw_name)
