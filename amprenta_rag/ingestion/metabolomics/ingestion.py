    ...
    if cfg.pipeline.enable_notion_sync or cfg.pipeline.enable_dual_write:
        try:
            if notion_page_id:
                ...
                logger.info(
                    "[INGEST][METABOLOMICS] Using existing Notion dataset page %s",
                    page_id,
                )
                # Update summary
                try:
                ...
                if postgres_dataset:
                    ...
                    db.commit()
                    logger.info(
                        "[INGEST][METABOLOMICS] Linked Postgres dataset %s to Notion page %s",
                        postgres_dataset.id,
                        page_id,
                    )
            ...
            logger.warning(
                "[INGEST][METABOLOMICS] Notion sync skipped (error): %r",
                e,
            )
    elif not cfg.pipeline.use_postgres_as_sot:
