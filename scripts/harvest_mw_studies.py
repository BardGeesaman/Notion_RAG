    ...
    try:
        ...
        if "application/json" in content_type:
            try:
                ...
                logger.debug("[MW] mwTab response looks like JSON but parse failed, using raw text")
                return text_content
        ...
        try:
            ...
            logger.warning("[MW] Error fetching data from alternative endpoint for %s: %r", study_id, e2)
...
def extract_mw_metadata(summary: Dict[str, Any]) -> Dict[str, Any]:
    ...
    result["title"] = (
        summary.get("study_title") or 
        summary.get("title") or 
        summary.get("study_name") or
        summary.get("name") or
        "(Untitled Study)"
    )
    
    result["organism"] = (
        summary.get("organism") or
        summary.get("species") or
        summary.get("sample_species") or
        ""
    )
    
    result["sample_type"] = (
        summary.get("sample_type") or
        summary.get("sample_source") or
        summary.get("matrix") or
        ""
    )
    
    result["disease"] = (
        summary.get("disease") or
        summary.get("condition") or
        summary.get("disease_condition") or
        ""
    )
    
    result["summary"] = (
        summary.get("study_summary") or
    ...
    result["pubmed_id"] = summary.get("pubmed_id") or summary.get("pubmed") or summary.get("pmid") or ""
...
def find_existing_dataset_page(study_id: str) -> Optional[str]:
    ...
    possible_property_names = ["MW Study ID", "MW Study", "Study ID", "Metabolomics Workbench Study ID"]
    ...
    try:
        ...
        if hasattr(e, 'response') and e.response is not None:
            logger.warning("[MW] Error querying for existing page: %s - Response: %s", str(e), e.response.text)
        else:
...
def create_dataset_page(study_id: str, mw_meta: Dict[str, Any], mwtab_text: str) -> str:
    ...
    try:
        ...
        if hasattr(e, 'response') and e.response is not None:
            logger.error("[MW] Error creating Dataset page for %s: %s - Response: %s", study_id, str(e), e.response.text)
        else:
...
def update_dataset_page(page_id: str, mw_meta: Dict[str, Any]) -> None:
    ...
    if props:
        ...
        try:
            ...
            if hasattr(e, 'response') and e.response is not None:
                logger.error("[MW] Error updating Dataset page %s: %s - Response: %s", page_id, str(e), e.response.text)
            else:
...
def add_mwtab_block(page_id: str, mwtab_text: str) -> None:
    ...
    while len(remaining) > max_block_length:
        ...
        if last_newline > max_block_length * 0.8:  # Only split at newline if we're not losing too much
    ...
    logger.info("[MW] Splitting mwTab into %d code block(s) for page %s", len(chunks), page_id)
    ...
    for i in range(0, len(blocks), batch_size):
        ...
        try:
            ...
            if i == 0:
                logger.info("[MW] Added mwTab heading and first batch of blocks to page %s", page_id)
            else:
                logger.info("[MW] Added batch %d of mwTab blocks to page %s", (i // batch_size) + 1, page_id)
        except Exception as e:
            if hasattr(e, 'response') and e.response is not None:
                logger.error("[MW] Error adding mwTab blocks to page %s (batch %d): %s - Response: %s", 
                            page_id, (i // batch_size) + 1, str(e), e.response.text)
            else:
                logger.error("[MW] Error adding mwTab blocks to page %s (batch %d): %r", 
                            page_id, (i // batch_size) + 1, e)
            # Continue with remaining batches even if one fails


def _search_mw_study_ids_by_keyword(keyword: str, summaries: Optional[List[Dict[str, Any]]] = None) -> List[str]:
    """
    ...
    for row in summaries:
        ...
        summary_text = (
            row.get("study_summary")
            or row.get("Study_summary")
            or ""
        )
        disease = (
            row.get("disease")
...
def _study_contains_lipids_matching_filters(study_id: str, lipid_filters: List[str]) -> bool:
    """
    ...
    for lf in lipid_filters:
        ...
        try:
            ...
            if resp.status_code != 200:
                logger.debug("[MW] RefMet endpoint returned %d for %s", resp.status_code, refmet_name)
            ...
            for line in lines[1:]:  # Skip header row
                ...
                if len(parts) >= 3:
                    ...
                    if found_study_id == study_id_upper:
                        logger.debug("[MW] Study %s found in RefMet data for %s", study_id, refmet_name)
                        return True
        except Exception as e:
            logger.debug("[MW] Error checking RefMet endpoint for %s: %r", refmet_name, e)
            continue
...
def harvest_study(
    ...
    try:
        ...
        try:
            ...
            logger.warning("[MW] Could not fetch mwTab for %s, continuing without it: %r", study_id, e)
            mwtab_text = ""
        
        if existing_page_id:
            logger.info("[MW] Found existing page %s for study %s", existing_page_id, study_id)
