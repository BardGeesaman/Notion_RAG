...
def _render_archived_tab() -> None:
    ...
    with db_session() as db:
        ...
        if entity_type_filter in ["All", "Experiments"]:
            archived_experiments = db.query(Experiment).filter(Experiment.is_archived).order_by(Experiment.archived_at.desc()).all()

        if entity_type_filter in ["All", "Datasets"]:
            archived_datasets = db.query(Dataset).filter(Dataset.is_archived).order_by(Dataset.archived_at.desc()).all()
        ...
        if archive_entity_type == "Experiment":
            experiments = db.query(Experiment).filter(not Experiment.is_archived).order_by(Experiment.name).limit(100).all()
            if experiments:
            ...
            datasets = db.query(Dataset).filter(not Dataset.is_archived).order_by(Dataset.name).limit(100).all()
            if datasets:
