...
def _render_browse_tab() -> None:
    ...
    with db_session() as db:
        ...
        if not show_inactive:
            query = query.filter(OntologyTerm.is_active == True)
    ...
    with db_session() as db:
        ...
        with st.form("add_ontology_term", clear_on_submit=True):
            ...
            if link_parent:
                ...
                existing_terms = db.query(OntologyTerm).filter(OntologyTerm.is_active == True).order_by(OntologyTerm.vocabulary, OntologyTerm.term).all()
                if existing_terms:
