#!/usr/bin/env python3
"""
CLI for ingesting internal polar metabolomics datasets.

Usage:
    python scripts/ingest_metabolomics.py --file <path> --create-page
    python scripts/ingest_metabolomics.py --file <path> --dataset-page-id <id>
    python scripts/ingest_metabolomics.py --file <path> --create-page --program-id <id> --experiment-id <id>
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Ingest an internal polar metabolomics dataset file into Notion and RAG."
    )
    parser.add_argument(
        "--file",
        required=True,
        help="Path to the metabolomics file (CSV/TSV)",
    )
    parser.add_argument(
        "--dataset-page-id",
        default=None,
    ...
    try:
        ...
        logger.error("[INGEST][METABOLOMICS] Ingestion failed: %r", e)
