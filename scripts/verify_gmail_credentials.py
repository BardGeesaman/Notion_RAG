...
def main():
    ...
    if not creds_path.exists():
        ...
        print(f"   {creds_path}")
        print(f"\nPlease:")
        print(f"1. Download OAuth2 credentials from Google Cloud Console")
    ...
    try:
        ...
        if "installed" in creds:
            ...
            if client_id and client_secret:
                ...
                print(f"   python scripts/setup_gmail.py")
                return 0
            else:
                print(f"\n⚠️  Credentials file missing required fields")
                print(f"   Expected: installed.client_id and installed.client_secret")
                sys.exit(1)
        else:
