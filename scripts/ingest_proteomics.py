#!/usr/bin/env python3
"""
CLI for ingesting internal proteomics datasets.

Usage:
    python scripts/ingest_proteomics.py --file <path> --create-page
    python scripts/ingest_proteomics.py --file <path> --dataset-page-id <id>
    python scripts/ingest_proteomics.py --file <path> --create-page --program-id <id> --experiment-id <id>
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Ingest an internal proteomics dataset file into Notion and RAG."
    )
    parser.add_argument(
        "--file",
        required=True,
        help="Path to the proteomics file (CSV/TSV)",
    )
    parser.add_argument(
        "--dataset-page-id",
        default=None,
    ...
    try:
        ...
        logger.error("[INGEST][PROTEOMICS] Ingestion failed: %r", e)
