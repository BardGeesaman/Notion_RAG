#!/usr/bin/env python3
"""
CLI for ingesting internal transcriptomics datasets.

Usage:
    python scripts/ingest_transcriptomics.py --file <path> --create-page
    python scripts/ingest_transcriptomics.py --file <path> --dataset-page-id <id>
    python scripts/ingest_transcriptomics.py --file <path> --create-page --program-id <id> --experiment-id <id>
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Ingest an internal transcriptomics dataset file into Notion and RAG."
    )
    parser.add_argument(
        "--file",
        required=True,
        help="Path to the transcriptomics file (CSV/TSV)",
    )
    parser.add_argument(
        "--dataset-page-id",
        default=None,
    ...
    try:
        ...
        logger.error("[INGEST][TRANSCRIPTOMICS] Ingestion failed: %r", e)
